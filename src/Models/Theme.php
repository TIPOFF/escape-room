...
class Theme extends Model
{
    ...
    {
        return $this->hasManyThrough(config('support.model_class.location'), Room::class, 'theme_id', 'id', 'id', 'location_id');
    }
    ...
    {
        return $this->belongsToMany(config('support.model_class.image'))->withTimestamps();
    }
    ...
    {
        return $this->belongsTo(config('support.model_class.image'), 'icon_id');
    }

    public function poster()
    {
        return $this->belongsTo(config('support.model_class.image'), 'poster_image_id');
    }

    public function video()
    {
        return $this->belongsTo(config('support.model_class.video'));
    }
    ...
    public function getPathAttribute()
    {
        if (Session::get('current_market_id') != null) {
            $market = config('support.model_class.market')::find(Session::get('current_market_id'))->slug;
    ...
    public function findMarkets()
    {
        ...
        $locations = config('support.model_class.location')::whereIn('id', $rooms->pluck('location_id'))
            ->whereNull('closed_at')
            ->get();

        $markets = config('support.model_class.market')::whereIn('id', $locations->pluck('market_id'))
            ->orderBy('state')
            ->orderBy('name')
            ->get();
